...
import com.apm.agent.context.ids.DistributedTraceId;
import com.apm.agent.context.ids.PropagatedTraceId;
import com.apm.agent.context.trace.*;
...
public class TracingContext implements AbstractTracerContext {
    private static final Logger logger = LoggerFactory.getLogger(TracingContext.class);
    ...
    private boolean isLimitMechanismWorking() {
        if (spanIdGenerator >= Config.Agent.SPAN_LIMIT_PER_SEGMENT) {
            ...
            if (currentTimeMillis - lastWarningTimestamp > 30 * 1000) {
                logger.warn("More than {} spans required to create", Config.Agent.SPAN_LIMIT_PER_SEGMENT,
